...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    foreach ($sportovi as &$s) {
        if ($s["id"] === $id) {
            ...
            // Ako je dodana nova glavna slika
            if (isset($_FILES["slika"]) && $_FILES["slika"]["error"] === UPLOAD_ERR_OK) {
                $ekstenzija = pathinfo($_FILES["slika"]["name"], PATHINFO_EXTENSION);
                $novoIme = "sport_" . time() . "." . $ekstenzija;
                if (move_uploaded_file($_FILES["slika"]["tmp_name"], $direktorijZaSlike . $novoIme)) {
                    $s["slika"] = "./images/" . $novoIme;
                }
            }

            // Ako su dodane nove slike u galeriju, dodajemo ih na postojeće
    ...
    if ($pronadjen) {
        ...
        echo json_encode(["poruka" => "Izmjene na sportu uspješno sačuvane!"]);
    } else {
